...
Settings can connect a portable configuration repository in three ways: create and publish a managed GitHub repository, clone an existing GitHub repository, or use an existing local Git repository. MyRepos stores the portable data in `.myrepos/config.json` and can commit/push changes automatically or pull, push, and sync manually. The file contains organization metadata, repository colors, assignments, workspace ordering, working-copy label preferences, SSH and file-transfer connection metadata and trusted fingerprints, and non-secret database access settings. Passwords, passphrases, OAuth tokens, private-key paths, agent sockets, command snippets and scripts, absolute local paths, and machine-specific availability are deliberately excluded.

The main-process remote filesystem layer provides a shared FTP, explicit/implicit FTPS, and SFTP abstraction. It supports connection testing, directory listing and metadata, bounded reads, writes, streaming upload/download with progress, directory creation, rename, deletion, and SFTP permissions. File-transfer profiles are configured from the SSH connections screen. SFTP can reuse a verified SSH profile or keep standalone password, private-key, or agent settings; FTP and FTPS use standalone profiles. Secrets are encrypted with the operating system credential store and remain local.
