...
const apacheSslInspectionCommand = [
  ...
  "for spec in '/etc/apache2/sites-available|0' '/etc/apache2/sites-enabled|1' '/etc/httpd/conf.d|1'; do directory=${spec%%|*}; enabled=${spec##*|}; [ -d \"$directory\" ] || continue; find \"$directory\" -mindepth 1 -maxdepth 1 \\( -type f -o -type l \\) -print 2>/dev/null | while IFS= read -r path; do target=$(readlink -f \"$path\" 2>/dev/null || printf '%s' \"$path\"); [ -f \"$target\" ] || continue; root=$(sed -nE 's/^[[:space:]]*DocumentRoot[[:space:]]+//Ip' \"$target\" 2>/dev/null | head -n1); root=${root#\\\"}; root=${root%\\\"}; root_exists=0; [ -n \"$root\" ] && [ -d \"$root\" ] && root_exists=1; printf 'site|%s|%s|%s|%s|%s\\n' \"$enabled\" \"$(b64 \"$(basename \"$path\")\")\" \"$(b64 \"$target\")\" \"$(base64 \"$target\" 2>/dev/null | tr -d '\\n')\" \"$root_exists\"; done; done",
